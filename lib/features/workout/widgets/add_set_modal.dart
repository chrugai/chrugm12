...
class _AddSetModalState extends State<AddSetModal> {
  ...
  void _record() {
    widget.onRecord(
      ...
      tempoVariation: _rir != null ? _tempo?.name : _tempo?.name,
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            BubbleSelector(
              options:
                  TempoVariation.values.map((t) => t.displayName).toList(),
              selectedOptions:
                  _tempo != null ? [_tempo!.displayName] : [],
              onSelected: (selected) {
                setState(() {
                  if (selected.isEmpty) {
                    _tempo = null;
